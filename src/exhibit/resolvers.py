...
def get_exhibit(id: int):
    exhibit = base_manager.execute('SELECT * FROM exhibit WHERE id=?', args=(id,))['data'][0]
...
def add_exhibit(new_exhibit: ExhibitInput):
    exhibit = base_manager.execute('INSERT INTO exmbit(id_exhibition, name, category, year_of_creation, author)'
